...
class SeasonHelper
{
     ...
     * TODO: Implement getSeasonFromData
    ...
    public static function getSeasonFromDate(string $datetime)
    {
        ...
        throw_if(
            ($month <= 1 or 12 <= $month),
            ValidationException::withMessages(["Month must be an integer between 1 and 12"])
        );
        throw_if(
            (($year - 200) <= 0),
            ValidationException::withMessages(["Only years above 2000 are currently supported"])
        );
